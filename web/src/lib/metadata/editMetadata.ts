import { bundleToExiftoolWriteArgs } from "@shared/bundleToExiftoolArgs.js";
...
export async function editSourceMetadata(
  ...
  const writeArgs = bundleToExiftoolWriteArgs(writeBundle);
  ...
  let working = await withExiftoolLock(() => writeMetadataWithExiftool(inputFile, writeArgs));
  ...
  if (motionPhoto && dirtyKeys.has("presentationTimestampUs")) {
    ...
    working = new Uint8Array(rebuildMotionPhotoXmpInJpeg(working.slice(), trailing.length));
