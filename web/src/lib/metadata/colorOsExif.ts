/**
 * ColorOS Hasselblad watermark requires II EXIF + Interop IFD + IFD0 YCbCrPositioning.
 * ExifTool TagsFromFile can still emit MM and drop Interop; segment transplant is preferred,
 * then syncFullExifFromSource when the container is not ColorOS-compatible.
 */
import { readExifByteOrder, type ExifByteOrder } from "./exifByteOrder";
...
export interface ColorOsExifValidateOptions {
  /** When set, also verify MotionPhoto XMP matches appended MP4 tail. */
  motionPhoto?: boolean;
...
function userCommentOk(tags: TagMap): boolean {
  const comment = tagValue(tags, "EXIF:UserComment", "UserComment");
  if (comment == null || comment === "") return true;
  return /^oplus_/i.test(String(comment));
}

/** Full ColorOS compatibility check (EXIF container + optional MotionPhoto XMP). */
export function validateColorOsExif(
  ...
  if (!userCommentOk(tags)) {
    issues.push("UserComment 缺少 oplus_ 前缀");
  }
  if (hasMpfApp2Segment(jpeg)) {
...
/** True when EXIF block is unlikely to pass ColorOS native metadata parsing. */
